function [U,B_k,V,p,ierr,work] = lanbproMN(A1,A2,m,n,k,p,U,B_k,V,anorm);
 ...
 %  
  ...
  alpha(1) = norm(r);
    if est_anorm
      ...
      r =  tprod(A1,A2,U(:,j),1,m,n) - beta(j)*V(:,j-1);
...
%    fro = 1;
 ...
 %   p = A*V(:,j) - alpha(j)*U(:,j);
     p = tprod(A1,A2,V(:,j),0,m,n)- alpha(j)*U(:,j);
  beta(j+1) = norm(p);
  % Extended local reorthogonalization
